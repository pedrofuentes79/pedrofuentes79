...
if [ $# -lt 2 ]; then
  echo "Usage: $0 <window_class> <url>"
  echo "Example: $0 chromium https://web.whatsapp.com/"
  exit 1
fi
...
window_address=$(hyprctl clients -j | jq -r --arg class "$WINDOW_CLASS" '.[] | select(.class == $class) | .address')
